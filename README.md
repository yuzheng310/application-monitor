...
当前源码提供 **22 个**校招或应聘记录入口：

百度 · MiniMax · 得物 · 快手 · 滴滴 · 美团 · 京东 · 蚂蚁集团 · 阿里巴巴 · 小红书 · 携程 · 米哈游 · 库洛游戏 · 商汤 · 同花顺 · 网易游戏 · 科大讯飞 · 小米 · 小鹏汽车 · Momenta · Shopee · 宁德时代（CATL）。

**版本差异：**v1.1.0 安装包已包含并行查询与错误反馈；小鹏汽车、Momenta、Shopee、宁德时代、更新优先排序、公司目录及本次视觉更新仍属于后续源码更新。现有用户配置不会被新版示例配置自动覆盖，新增公司需合并到自己的 `sites.json`，可让 AI 按指南完成。
...
并行查询期间，软件会在自己的查询窗口内轮换激活标签页，让依赖可见状态的官网完成渲染，再读取记录；网页加载仍可并行。查询失败不会覆盖成功记录，也不表示被拒绝。日志位于 `data/web-server.log` 和 `data/web-query.log`，公开反馈前需脱敏。
