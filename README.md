...
小鹏汽车、Momenta 入口及更新优先排序尚未包含在 v1.1.0 安装包中，将随后续版本发布。
...
带「记录有更新」标记的公司自动排在前面，同组内继续按岗位进度排序。网页默认展示非实习岗位，保留已结束的非实习记录。流程名称不代表已完成；只有明确的状态、日期或高亮证据才用于判断。失败不会覆盖上一次成功记录，也不会推断为被拒绝。当前不自动翻页或遍历所有招聘年份，最终结果以官网和 HR 通知为准。
