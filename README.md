...
当前源码提供 **28 个**校招或应聘记录入口：

百度 · MiniMax · 得物 · 快手 · 滴滴 · 美团 · 京东 · 蚂蚁集团 · 阿里巴巴 · 小红书 · 携程 · 米哈游 · 库洛游戏 · 商汤 · 同花顺 · 网易游戏 · 科大讯飞 · 小米 · 小鹏汽车 · Momenta · Shopee · 宁德时代（CATL） · 拼多多集团（PDD） · 微软（Microsoft） · 顺丰 · 智元机器人（AGIBOT） · 拓竹科技（Bambu Lab） · 上海人工智能实验室。

**版本差异：**v1.1.0 安装包已包含并行查询与错误反馈；小鹏汽车、Momenta、Shopee、宁德时代、拼多多、微软、顺丰、智元机器人、拓竹科技、上海人工智能实验室、更新优先排序、公司目录及本次视觉更新仍属于后续源码更新。现有用户配置不会被新版示例配置自动覆盖，新增公司需合并到自己的 `sites.json`，可让 AI 按指南完成。

读取规则基于招聘网页，不是官方接口。页面改版、登录失效、空记录或额外验证都可能导致查询失败。软件不提交、修改或撤回申请；当前只展示非实习岗位，不自动翻页或遍历所有招聘年份。状态以招聘官网和 HR 通知为准。

## 桌面窗口

macOS 的 `投递进度助手.app` 使用系统 WebKit 独立窗口，应用持续运行直到关闭窗口或退出；启动失败会显示重试提示。官网链接仍交给查询浏览器打开。构建此入口需要 Xcode Command Line Tools，使用者无需安装开发工具。请保留整个解压目录，不要单独移走这个入口。Windows 和 Linux 继续使用原启动文件，通过内置浏览器的应用窗口显示看板。可用 `--browser-tab` 恢复普通标签页，`--no-browser` 仅启动本地服务。关闭窗口不会停止已有后台查询服务，重新打开仍使用原来的记录和排期。此改动尚未发布为新安装包。

快手的简历筛选会读取感叹号悬浮提示，区分 HR 初筛和用人部门筛选，并保留官网明确标注的完成情况。无法辨认的图标不会推断为通过。
